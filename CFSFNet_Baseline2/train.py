...
import argparse
from utils import *
from loss import hcc, ptcc, CenterTripletLoss
...
parser.add_argument('--max_epoch', '-maxe', default=200, type=int, help='maximum epochs')#200 for sysu, 140 for regdb
parser.add_argument('--warmup_epoch', '-ware', default=10, type=int, help='>=0')
parser.add_argument('--scheduler_epoch', '-sche', default=[80, 140], type=int, nargs='*',
                    help='lr scheduler by Step')#80 140 for sysu , 80 120 for regdb
parser.add_argument('--test_epoch', default=5, type=int, help='test model every 10 epochs, not equal to real test')
...
criterion_tct =tct_Loss(margin=0.7)
criterion_tct.to('cuda')
criterion_tri = CenterTripletLoss(margin=0.3)
...
net = embed_net(class_num=dataset.num_train_pids)
net.to('cuda')
...
# transform_train, transform_test = get_transform_naive(args)
...
def train(epoch):
    ...
    for batch_idx, (imgs, pids, camids) in enumerate(trainloader):
        ...
        # imgs = imgs.cuda()
        input1, input2 = torch.chunk(imgs, 2)
        input1 = input1.cuda()
        input2 = input2.cuda()
        pids = pids.cuda()
        data_time.update(time.time() - end)
        end = time.time()
        pool, y, inputs = net(input1,input2)
        ...
        loss_tct = (criterion_tct(inputs10, inputs11, pids) + criterion_tct(inputs20, inputs21, pids)
                    + criterion_tct(inputs30, inputs31, pids) + criterion_tct(inputs40, inputs41, pids)
                    + criterion_tct(inputs50, inputs51, pids) + criterion_tct(inputs60, inputs61, pids)) / 6
        ...
        #     random.shuffle(rdn_list)
        #     select = torch.arange(args.part_num)
        #     select[rdn_list[args.part_num//2:]] += args.part_num #
        #     mix_imgs.append(imgs_list[i, :, select.type(torch.long), :, :].view(1, imgs.size(1), imgs.size(2), imgs.size(3)))
        # mix_imgs = torch.cat(mix_imgs, 0)
        # mix_imgs = mix_imgs.cuda()
        ...
        tct_loss.update(loss_tct.item(), pids.size(0))
        ...
        if (batch_idx + 1) % (len(trainloader) // 10) == 0:
            print(f'E[{epoch:02d}][{batch_idx + 1:3d}/{len(trainloader)}] '
                  ...
                  f'L_tct: {tct_loss.val:.3f} ({tct_loss.avg:.3f}) '
...
def test(net, queryloader, galleryloader,reg_itov=0):
    if reg_itov == 0:
        q_pool, q_feat, q_pids, q_camids = extract_feature(net, queryloader, test_mode[1])  # sysu 2 ir,reg 1 vis
        g_pool, g_feat, g_pids, g_camids = extract_feature(net, galleryloader, test_mode[0])
        ...
        q_pool, q_feat, q_pids, q_camids = extract_feature(net, queryloader, test_mode[0])  # reg 2 inf
        g_pool, g_feat, g_pids, g_camids = extract_feature(net, galleryloader, test_mode[1])
        q = F.normalize(q_feat, p=2, dim=1)
        g = F.normalize(g_feat, p=2, dim=1)
        distmat = -torch.mm(q, g.t()).numpy()
        cmc1, mAP1, mINP1 = eval(distmat, q_pids, g_pids, q_camids, g_camids)
    ...
    if epoch % args.test_epoch == 0 or epoch == args.max_epoch:
        ...
        if args.dataset == 'sysu':  ##################################################################################################
            ...
            for trial in range(t):
                ...
                cmc_t, mAP_t, mINP_t = test(net, queryloader, galleryloader,0)
                if trial == 0:
            ...
            cmc, mAP, mINP = test(net, galleryloader,queryloader,0)
            print(f'Rank-1: {cmc[0]:.2%} | Rank-5: {cmc[4]:.2%} | Rank-10: {cmc[9]:.2%} | '
                  f'Rank-20: {cmc[19]:.2%} | mAP: {mAP:.2%} | mINP: {mINP:.2%} '
                  f'Evaluation Time: {time.time() - start:.3f}')
            tmp_best_r1 = cmc[0]
            ...
            cmc, mAP, mINP = test(net, queryloader, galleryloader, 1)
            print(f'Rank-1: {cmc[0]:.2%} | Rank-5: {cmc[4]:.2%} | Ra'
                  f'nk-10: {cmc[9]:.2%} | '
                  f'Rank-20: {cmc[19]:.2%} | mAP: {mAP:.2%} | mINP: {mINP:.2%} '
                  f'Evaluation Time: {time.time() - start:.3f}')
