...
parser.add_argument('--resume', '-r', default='modelpath', type=str,
...
parser.add_argument('--method', default='new', type=str,
                    metavar='m', help='method type: base or new')
...
if args.method =='base':
    net = embed_net(n_class, no_local= 'off', gm_pool =  'off', arch=args.arch, share_net=args.share_net, pcb=args.pcb, local_feat_dim=args.local_feat_dim, num_strips=args.num_strips)
else:
    net = embed_net(n_class, no_local= 'on', gm_pool = 'on', arch=args.arch, share_net=args.share_net, pcb=args.pcb, local_feat_dim=args.local_feat_dim, num_strips=args.num_strips)
net.to(device)    
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        ...
        model_path = checkpoint_path + ''.format(test_trial)
        if os.path.isfile(model_path):
            # print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
